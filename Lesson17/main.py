...
while cap.isOpened():
    ...
    # 1 = flipped horizontally, 0 = vertical
    ...
    if (results.multi_hand_landmarks):
        for hand, handedness_info in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            gesture = detectGesture(hand, handedness)
            mp_drawing.draw_landmarks(img, hand, mp_hands.HAND_CONNECTIONS)
            if (time.time() - last_scroll) > SCROLL_DELAY:
                if (gesture == "scroll_up"): pyautogui.scroll(SCROLL_SPD)
                elif (gesture == "scroll_down"): pyautogui.scroll(-SCROLL_SPD)
