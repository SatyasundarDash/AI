...
def senti(p): return "Positive :D" if p > 0 else "Negative D:" if p < 0 else "Neutral :|"
...
def show(recs, name):
    ...
    for i, (t, p) in enumerate(recs, 1): print(f"{Fore.CYAN}{i}. {t} (Polarity: {p:.2f}, {senti(p)})")
...
md = "postivie :D" if mp > 0 else "negative D:" if mp < 0 else "neutral :|"
...
while True:
    a = input(f"{Fore.YELLOW}\nWould you like more recommendations? (yes/no): ").strip().lower()
    if (a == "no"):
        print(Fore.GREEN + f"\nEnjoy your movie picks, {name}: \n"); break
    if (a == "yes"):
        recs = recommend(genre=genre, mood=mood, rating=rating, n=5)
        print(Fore.RED + recs + "\n") if isinstance(recs, str) else show(recs, name)
    else: print(f"{Fore.RED}Invalid choice, try again. \n")
